...
Under the null (\(\delta = 0\)), all tested designs held empirical type
I error within Monte Carlo tolerance of the nominal \(\alpha = 0.05\):
rejection rates ranged from 0.039 to 0.058 (MCSE \(\leq\) 0.005). At
\(\delta = 0.5\), the fully sequential design achieved rejection rate
0.999 with mean sample size 91.0 (MCSE 0.61), versus the fixed design's
sample size of 200 at rejection rate 0.999. All performance measures
include Morris Table 6 Monte Carlo SEs in the tables below.
...
\emph{Source:
\texttt{\textasciitilde{}/prj/res/20-sequential-analysis/sequentialanalysis/analysis/report/report.Rmd}}
